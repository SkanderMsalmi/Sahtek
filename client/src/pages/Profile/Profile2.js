...
function Profile2(){
  ...
  const [edit, setEdit] = useState(false);
    const { data, loading, error,refetch } = useQuery(USER_PROFILE, {
        variables: { id:id? id: user.id },},{notifyOnNetworkStatusChange: true});
        const initialValues = {
...
// Will display time in 10:30:23 format
  ...
  const [dateOfBirth, setDateOfBirth] = useState(new Date(data?.user.dateOfBirth*1));
  ...
  if(error) return <p>{error}</p>
    return (
        ...
        <div className="section profile-content">
          <Container>
            <div className="owner">
            {edit?( loading? "loading..."  :   <EditUser user={{name,dateOfBirth,pdp,profileImage:profileImage}} refetch={refetch} setProfileImage={setProfileImage} setName={setName} setDateOfBirth={setDateOfBirth} setPdp={setPdp} setEdit={setEdit} id={user.id}/>
                ...
                <h6 className="description">{getAge(dateOfBirth)} {data?.user?.role}</h6>
            ...
            <div className="nav-tabs-navigation">
              <div className="nav-tabs-wrapper">
                <Nav role="tablist" tabs>
                  ...
                  ):(data.user.role==="Therapist"?( <NavItem>
                    <NavLink
                      className={activeTab === "2" ? "active" : ""}
                      onClick={() => {
                        toggle("2");
    ...
    <Button className="btn-round" color="warning">
      Take test
    </Button></div>)):(data.user.role==="Therapist"?(<Therapist email={data.user.email}/> ):(<></>))}
