    ...
    return (
        <VideoChat>
            ...
            {!ready && <Row className="mb-2" style={{ display: "inline", width: "100%", textAlign: "center", color: "whitesmoke" }}>
                {(remoteEmail && !remoteStream && (remoteEmail !== user.email && (remoteEmail === data.getAppointment.patient.email || remoteEmail === data.getAppointment.therapist.email))) && <h4>You are connected to {user.name === data.getAppointment.patient.name ? "Dr. " + data.getAppointment.therapist.name : data.getAppointment.patient.name}</h4>}
                ...
                {(remoteEmail && remoteStream) && <h4>{user.name === data.getAppointment.patient.name ? "Dr. " + data.getAppointment.therapist.name : data.getAppointment.patient.name} is waiting for you to join</h4>}
