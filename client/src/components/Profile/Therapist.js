...
function Therapist(props){
    ...
    const handleSubmit = (e) => {
        ...
        setEditInfo({...editInfo, specialties: cleanArray(editInfo.specialties), languages: cleanArray(editInfo.languages), education: cleanArray(editInfo.education)});
            updateTherapist({variables: {therapistInput: {...editInfo,availability: editInfo.availability,licenses: editInfo.licenses, specialties: cleanArray(editInfo.specialties), languages: cleanArray(editInfo.languages), education: cleanArray(editInfo.education)}}}).then((res) => {
                setEdit(false)
            })
    ...
    function cleanUpAvailability(array) {
        ...
        array.forEach((item) => {
            if (item.startTime === lastCase.startTime && item.endTime === lastCase.endTime) {
                lastCase = {...lastCase,day: lastCase.day.replace(/\-.*/, '')+"-"+item.day};
            }
            else {
                if (lastCase.day !== "") {
    ...
    <div className="row">
        <div className="col-md-6">
            <div className="card" style={{height:'100%'}}>
                <div className={`${styles.contactInfo} card-body`} >
                    ...
                    {edit?  <ul style={{padding:"0"}}>
                        <li>
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path d="M164.9 24.6c-7.7-18.6-28-28.5-47.4-23.2l-88 24C12.1 30.2 0 46 0 64C0 311.4 200.6 512 448 512c18 0 33.8-12.1 38.6-29.5l24-88c5.3-19.4-4.6-39.7-23.2-47.4l-96-40c-16.3-6.8-35.2-2.1-46.3 11.6L304.7 368C234.3 334.7 177.3 277.7 144 207.3L193.3 167c13.7-11.2 18.4-30 11.6-46.3l-40-96z"/></svg><input  className={` form-control ${styles.borderless}`} type="text" name="phoneNumber" placeholder="Phone number" value={editInfo.phoneNumber} onChange={(e) => handlePhone(e)} />
                        ...
                        <li>
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512"><path d="M215.7 499.2C267 435 384 279.4 384 192C384 86 298 0 192 0S0 86 0 192c0 87.4 117 243 168.3 307.2c12.3 15.3 35.1 15.3 47.4 0zM192 128a64 64 0 1 1 0 128 64 64 0 1 1 0-128z"/></svg><div style={{display:"flex"}}> <input  className={` form-control ${styles.borderless}`} type="text" name="street" placeholder="Street" value={editInfo?.address?.street} onChange={(e)=> setEditInfo({...editInfo,address:{...editInfo.address,street: e.target.value}})}/> <input  className={` form-control ${styles.borderless}`} type="text" placeholder="zip" name="zip" value={editInfo?.address?.zip} onChange={(e)=> setEditInfo({...editInfo,address:{...editInfo.address,zip: e.target.value}})}/> <input  className={` form-control ${styles.borderless}`} type="text" name="city" placeholder="city" value={editInfo?.address?.city} onChange={(e)=> setEditInfo({...editInfo,address:{...editInfo.address,city: e.target.value}})}/> <input  className={` form-control ${styles.borderless}`} type="text" name="state" placeholder="state" value={editInfo?.address?.state} onChange={(e)=> setEditInfo({...editInfo,address:{...editInfo.address,state: e.target.value}})}/></div>
                        </li>
                        ...
                        <div className="row">
                            ...
                            <div className="row">
                                <div className="col-md-8">
                                    ...
                                    <h5>Fees</h5>
                                  {edit? <><input className={` form-control ${styles.borderless}`} type="text" name="fees" value={editInfo.fees} onChange={(e)=>handleFees(e)} /><br/></>: <p>{editInfo.fees}<small>TND</small></p>}  
       ...
       {edit?  <div  className={`${styles.contactInfo} col-md-6`}>
            ...
            <h5>Biography</h5><span>
              ...
              {user.id===data?.user.id? <><Button className="btn-round" style={{marginRight:"1rem"}} onClick={(e)=> handleSubmit(e)} color="success">Save</Button><i onClick={()=>setEdit(!edit)} className={`${styles.gear} fa-solid fa-gear`}></i></>:"" }  </span>
            </div>
...
<div style={{display:"flex",justifyContent:"space-between"}} className="mb-2 mt-2">
            <h5>Education</h5>
...
<input  className={` form-control ${styles.borderless}`} type="text" name="education" value={edu} onChange={(e)=>handleEducation(e,i)} />
                    <div className="input-group-append">
                      <button className={`btn btn-outline-danger ${styles.btnTrans}`} type="button" onClick={()=>setEditInfo({...editInfo, education:editInfo.education.filter(e=>e!=editInfo.education[i])}) }>🗑️</button>
                    </div>
                  </div>
                })}
            ...
            <p>{editInfo.languages.map((l,i)=>{return <div className="input-group mb-3">
            <input  className={` form-control ${styles.borderless}`} type="text" name="language" value={l} onChange={(e)=>handleLanguage(e,i)}/>
                                <div className="input-group-append">
                      <button className={`btn btn-outline-danger ${styles.btnTrans}`} type="button" onClick={()=>setEditInfo({...editInfo, languages:editInfo.languages.filter(l=>l!=editInfo.languages[i])}) }>🗑️</button>
                    </div>
                  </div>
                    })}</p>
            <h5>Years of experience</h5>
            <p>{getExperience(new Date(data?.user?.therapist.experience*1))} </p>
            </div>
            <div  className={`${styles.contactInfo} col-md-6`}>
            <div style={{display:"flex",justifyContent:"space-between"}} className="mb-2 mt-2">
            ...
            <ul style={{padding:0}}>
                {editInfo.specialties.map((s,i) => {
                    return <div className="input-group mb-3">
<input  className={` form-control ${styles.borderless}`} type="text" name="specialty" value={s} onChange={(e)=>handleSpecialty(e,i)}/>                                        <div className="input-group-append">
                              <button className={`btn btn-outline-danger ${styles.btnTrans}`} type="button" onClick={()=>setEditInfo({...editInfo, specialties:editInfo.specialties.filter(s=>s!=editInfo.specialties[i])}) }>🗑️</button>
                            </div>
                          </div>
                })}
            </ul>
            ...
            <div style={{display:"flex",alignContent:"center",justifyContent:"space-between"}}>
            <h5>Biography</h5>
            {user.id===data?.user.id? <i onClick={()=>setEdit(!edit)} className={`${styles.gear} fa-solid fa-gear`}></i>:""}
            </div>
